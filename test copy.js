            ...
            superagent('GET',url).retry(10).timeout({
                ...
                superagent('GET', item).retry(10).timeout({
                    ...
                    $(paper).find('p', '.newsCon').each((i, item) => {
                        ...
                        // 采购人
                        if($(item).text().indexOf('采购人：') != -1 && flag1 === 0){
                            let regex = /采购人\S+\s?/
                            if($(item).text().match(regex) != null)
                            {
                                people = $(item).text().match(regex)[0].trim().replace("采购人：", "")
                            }
                        }
                        else if($(item).text().indexOf('采购人名称：') != -1 && flag1 === 0){
                            let regex = /采购人名称\S+\s?/
                            if($(item).text().match(regex) != null)
                            {
                                people = $(item).text().match(regex)[0].trim().replace("采购人名称：", "")
                            }
                        }
                        else if($(item).text().indexOf('采购人信息') != -1 && flag1 === 0){
                            flag1 = 1;
                        }
                        else if(flag1 === 1){
                            let regex = /名称\S+\s?/
                            if($(item).text().match(regex) != null)
                            {
                                people = $(item).text().match(regex)[0].trim().replace("名称：", "")
                            }
                            flag1 = 0;
                        }
                        // 采购代理机构
                        if($(item).text().indexOf('采购代理机构：') != -1 && flag2 == 0){
                            let regex = /采购代理机构\S+\s?/
                            if($(item).text().match(regex) != null)
                            {
                                company = $(item).text().match(regex)[0].trim().replace("采购代理机构：", "")
                            }
                        }
                        else if($(item).text().indexOf('采购代理机构名称：') != -1 && flag2 == 0){
                            let regex = /采购代理机构名称\S+\s?/
                            if($(item).text().match(regex) != null)
                            {
                                company = $(item).text().match(regex)[0].trim().replace("采购代理机构名称：", "")
                            }
                        }
                        else if($(item).text().indexOf('采购代理机构信息') != -1&&flag2==0){
                            flag2 = 1;
                        }
                        else if(flag2 == 1){
                            let regex = /名称\S+\s?/
                            if($(item).text().match(regex) != null)
                            {
                                company = $(item).text().match(regex)[0].trim().replace("名称：", "")
                            }
                            flag2 = 0;
                        }
                        if($(item).text().indexOf("提交投标文件截止时间、开标时间和地点")!==-1){
                            flag3 = 1
                        }
                        else if(flag3 === 1){
                            deadline = $(item).text()
                        ...
                        else if(flag3 === 2 && $(item).text().indexOf("时间") === -1){
                            address = $(item).text()
                            flag3 = 0
                        }
                    })
                    ...
                    exec(sql2).then(data => {
                        if(!data[0]){
                            //写入数据库
                        let sql = `INSERT INTO result (title, abstract, dateTime, body, budget, serialNum, people, company, attachment, area, deadline, address)VALUES
                        (${title}, ${abstract}, ${dateTime}, ${paperBody}, ${budget}, ${serialNum}, ${people}, ${company}, ${attachment}, '${t}', ${deadline}, ${address})`
                        exec(sql).then(res=>{
                            console.log('写入成功')
                        }).catch(err=>{
                            console.log(err)
